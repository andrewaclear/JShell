...
public class ListContents extends Command {
  ...
  /**
   * List the sub folders and files at the given path in the file system. 
   * Prints an error if the path does not exist in the file system.
   * 
   * @param token, string of a token of a path entered by the user, 
   * used to print as a heading to it's contents when given
   * @param fSystem, instance of FileSystem
   * @param path, string of a path to a folder, a file, or nothing (where an 
   * error message is printed)
   * @return returns a boolean true if it exits in the fSystem and has 
   * contents and false otherwise
   */
  private boolean listSubFoldersFiles(String token, FileSystem fSystem, String path) {
    ...
    if (node != null) {
      ...
      if (subFolders.size() > 0 || files.size() > 0) notEmpty = true;
    ...
    if (!notEmpty) ErrorHandler.badInput(this, 
   ...
   * current directory, with a new line following each of the content (file 
   * or directory).
  ...
  public boolean run(String[] tokens, FileSystem fSystem, Cache cache) {
    ...
    if (tokens.length == 1) {
      ...
      for (int i = 1; i < tokens.length && success; i++) {
        ...
        if (success && i!=1) StandardOutput.print("\n");
        // absolute path, current directory, or parent directory
        if (tokens[i].charAt(0) == '/' ||
            tokens[i].equals(".") ||
            tokens[i].equals("..")) path = tokens[i];
        // relative path
