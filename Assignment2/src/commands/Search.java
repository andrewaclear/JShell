      ...
      for (int iPath = 1; iPath < len - 4; iPath++) {
        ...
        if (node != null) {
          if (fParam) {
            ...
            for (int i = 0; i < n; i++) {
              found = node.getDirectory().getFiles().get(i).getFileName()
                  .equals(item);
              if (found) {
            ...
            if (!found)
              output +=
                  tokens[iPath] + ": does not contain file: " + name + "\n";
          } else {
            n = node.getChildren().size();
            for (int i = 0; i < n; i++) {
              found = node.getChildren().get(i).getDirectory()
                  .getDirectoryName().equals(item);
              if (found) {
                output +=
                    tokens[iPath] + ": contains directory: " + name + "\n";
                break;
              }
            }
            if (!found)
              output += tokens[iPath] + ": does not contain directory: " + name
                  + "\n";
          }
        } else {
